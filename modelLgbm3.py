...
class ModelLgbm:
    ...
    def test(self):
        ...
        data.sort_values(by=["date", "res"], ascending=[True, True], inplace=True)
    ...
    def calculate_test(self,name,start,end,):
        ...
        def process(d):
            ...
            a = d1.code
            d1 = d1.drop(columns=["code",'Unnamed: 0',"name","close"])
    ...
    def calculate_train(self,name,start,end,):
        ...
        def process(d):
            ...
            d = d.drop(columns=["code",'Unnamed: 0',"name","close"])
    ...
    def calculate(self,d):
        ...
        d["adrx_14_r"] = d.close/ta.ADXR(d.high, d.low, d.close, timeperiod=3)
        d["adrx_8_r"] = d.close/ta.ADXR(d.high, d.low, d.close, timeperiod=8)
        ...
        d['low_1'] = (d.close - d.low.shift(1))/d.close
        d["return"] = ((d.close-d.close.shift(1))/d.close)/(((d.close)-d.close.shift(3))/d.close)
        d["return2"] = ((d.close-d.close.shift(1))/d.close)/(((d.close)-d.close.shift(5))/d.close)
        d["return3"] = ((d.close-d.close.shift(5))/d.close)
        d["rsv"] = (d.close-d.low)/(d.high-d.low)
        d["k"],d["d"] = ta.STOCH(d['high'].values,d['low'].values, d['close'].values, fastk_period=9, slowk_period=3, slowk_matype=0, slowd_period=3, slowd_matype=0)
        d["k_seriers"] = d.k/d.k.rolling(3).mean()
        d["k_seriers5"] = d.k/d.k.rolling(5).mean()
        d["k_seriers7"] = d.k/d.k.rolling(7).mean()
        d["k_seriers15"] = d.k/d.k.rolling(15).mean()
        d["k_seriers30"] = d.k/d.k.rolling(30).mean()
        ...
        d = d.drop(columns=["open","high","low","volume","成交额","涨跌额"])
        d.to_csv("./data.csv")
        # print(d.head)
...
if __name__=="__main__":
    train = ModelLgbm(checkpoint="./checkpoint/labmres2.pt",test_name="沪深300")
